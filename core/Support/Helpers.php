...
if (!function_exists('url')) {
    /**
     * Génère une URL absolue à partir du chemin fourni.
     *
     * Exemple :
     *   url('/login') => https://goodbook.cd/login
     *   url('api/books') => https://goodbook.cd/api/books
     *
     * @param string $path Chemin relatif (avec ou sans slash initial)
     * @return string URL complète
     */
    function url(string $path = ''): string
    {
        // Récupération du domaine depuis .env ou déduction automatique
        $base = rtrim($_ENV['APP_URL'] ?? ($_SERVER['APP_URL'] ?? ''), '/');

        if ($base === '') {
            // Déduire automatiquement le schéma et l’hôte si non défini
            $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
            $host = $_SERVER['HTTP_HOST'] ?? 'localhost';
            $base = "{$scheme}://{$host}";
        }
